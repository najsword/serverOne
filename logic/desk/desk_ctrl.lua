...
local callbacks = {}
local player_agents = {}  -- player_id: room.agent
local player_ctxs = {}  -- player_id: ctx
...
function desk_ctrl.login_desk(ctx, agent)
	...
	if callbacks[DESK_CALLBACK.login_desk] then
		local ec = callbacks[DESK_CALLBACK.login_desk](ctx)
...
function desk_ctrl.logout_desk(ctx)
	if callbacks[DESK_CALLBACK.logout_desk] then
		local ec = callbacks[DESK_CALLBACK.logout_desk](ctx)
		if ec ~= SYSTEM_ERROR.success then
			return ec
		end
	end
...
function desk_ctrl.player_disconnect()
	if callbacks[DESK_CALLBACK.player_disconnect] then
		local ec = callbacks[DESK_CALLBACK.player_disconnect](ctx)
		if ec ~= SYSTEM_ERROR.success then
			return ec
		end
	end
...
function desk_ctrl.player_reconnect(ctx)
	if callbacks[DESK_CALLBACK.player_reconnect] then
		local ec = callbacks[DESK_CALLBACK.player_reconnect](ctx)
		if ec ~= SYSTEM_ERROR.success then
			return ec
		end
	end
...
function desk_ctrl.can_group()
	if callbacks[DESK_CALLBACK.can_group] then
		local ec = callbacks[DESK_CALLBACK.can_group]()
		return ec
	end
	return false
end
