...
local FdPlayerIdKey = "fd_playerId"
...
-- fd - playerid 无用
function hall_db.get_fd_playerid(fd)
	local redisdb = db_mgr.get_redis_db()
	return redisdb:hget(FdPlayerIdKey, fd)
end

function hall_db.set_fd_playerid(fd, playerId)
	local redisdb = db_mgr.get_redis_db()
	redisdb:hmset(FdPlayerIdKey, fd, playerId)
end
