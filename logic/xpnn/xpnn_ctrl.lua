...
local desk_id
local game_state
local player_map = {}  -- seat:player
local seat_state_map = {}  -- seat：{seat = seat, state = SEAT_STATE.ready } 
local ready_begin_cd_id = nil
local game_end_cd_id = nil
local exit_desk_cd_id = nil
...
local deal_id
local table_base = {}  --无用
local player_cards_map = {}  -- seat：{seat = seat, cards = {i}, card_type =i }
local banker = nil
local qiang_times_map = {}  -- seat: qiang_times
local bet_times_map= {}  -- seat: beat_times
local winlost_map = {}  -- banker: { seat = banker, winlost = 0, fee = 0}
local open_card_map = {}  -- seat: true/false

local unshow_cards = {}
local group_cards_map = {} -- seat: max_card
local max_card_map = {}   -- seat: max_card
local payout_end_map = {}  -- player_id: true/false
...
function xpnn_ctrl.is_player_online(seat)
	...
	if seat_state_map[seat].state == SEAT_STATE.offline  or seat_state_map[seat].state == SEAT_STATE.exit then
		return false
	end
	return true
end
...
function xpnn_ctrl.is_player_gaming(seat)
	...
	if seat_state_map[seat].state == SEAT_STATE.gaming  then
		return true
	end
	return false
end
...
------------- desk_ctrl callback  --------------------------
function xpnn_ctrl.login_desk(ctx)
...
function xpnn_ctrl.logout_desk(ctx)
	...
	if seat == ERR_SEAT or seat_state_map[seat].state == SEAT_STATE.exit then
		return SYSTEM_ERROR.success
	end
	...
	seat_state_map[seat].state = SEAT_STATE.exit
...
local function on_game_end()
	...
	for seat, v in pairs(player_map) do
		local event = {}
		event.over = 1
		xpnn_ctrl.send_game_end_event(seat, event)
		seat_state_map[seat].state = SEAT_STATE.game_end
	end
	game_state = GAME_STATE.game_end
	exit_desk_cd_id = cd_ctrl.add_cd(GAME_STATE.exit, xpnn_desk_config[room_id].exit_time)
end

-- 解散玩家
local function on_exit_desk()
	exit_desk_cd_id = nil
	for seat, v in pairs(player_map) do
		if seat_state_map[seat].state ~= SEAT_STATE.exit then
			local event = {}
			xpnn_ctrl.send_exit_desk_event(seat, event)
			...
			if ctx ~= nil then
				...
				context.call(online.roomaddr, "logout_desk", ctx, nil)
				--desk_ctrl.logout_desk(ctx)
			end
		end
	end
...
function xpnn_ctrl.broad_seat_state_event(seat)
	local event = {}
	event.player_map = player_map[seat]
	event.seat_state_map = seat_state_map[seat]
	for k, v in pairs(player_map) do
...
function xpnn_ctrl.send_game_end_event(seat, event)
	if not xpnn_ctrl.is_player_online(seat) then
		return
	end
	local ctx = xpnn_ctrl.get_player_ctx_by_seat(seat)
...
function xpnn_ctrl.send_exit_desk_event(seat, event)
	if not xpnn_ctrl.is_player_online(seat) then
		return
	end
	local ctx = xpnn_ctrl.get_player_ctx_by_seat(seat)
	if ctx then
		context.send_client_event(ctx, M_XPNN.exit_desk_event, event)
	end
end
-------------------  xpnn logic ----------------------
...
function xpnn_ctrl.handle_game_totally_end()
	...
	for k, v in pairs(player_map) do
		-- 添加 屏蔽启动错误
		--xpnn_ctrl.send_game_end_event(k, )
	end
...
function xpnn_ctrl.qry_desk(ctx, req)
	...
	reply.table_info.table_base = { deal_id = deal_id, game_state = game_state }
...
function xpnn_ctrl.init(roomid)
	desk_id = skynet.self()
	game_state = GAME_STATE.ready_begin
	deal_id = 0
	room_id = roomid
	...
	desk_ctrl.register_callback(DESK_CALLBACK.can_group, xpnn_ctrl.can_group)
	...
	cd_ctrl.register_callback(GAME_STATE.exit, on_exit_desk)
end
