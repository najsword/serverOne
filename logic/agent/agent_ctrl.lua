...
function agent_ctrl.on_login(ctx, player_info)
	...
	if agent_ctrl.is_hall() then
		db_helper.call(DB_SERVICE.hall, "hall.update_player_online", player_id, "hall_agentnode", current_conf.nodename)
		...
		context.rpc_call(ctx.gate, ctx.watchdog, "set_agent", ctx.fd, skynet.self())
	end
end
