...
local cs = queue()
local incr_id = 0
local cache_cds = {}  -- incr_id : {incr_id, cd_type, end_time, args}
local cd_callbacks = {}  -- cd_type ：func
local cd_listeners = {}  -- [cd_type][ctx.player_id] = ctx
...
function cd_ctrl.add_cd(cd_type, seconds, ...)
	...
	skynet.timeout(seconds * 100, function() 
		handle_cd_result(cd_type, id, table.unpack(args)) 
	end)
