...
local function _disconnect_callback(conf)
	...
	--publish_redis_message("cluster_mgr.remove_cluster_node", str_conf)
...
-- 添加前置node等待条件
local function wait_node(nodename, conn)
	if string.find(nodename, 'db') ~= nil then
		while(true) do
			skynet.sleep(100)
...
function cluster_mgr.open()
	local current_conf = cluster_mgr.get_current_conf()
	skynet.error("cy==========================cluster nodename=", current_conf.nodename)
