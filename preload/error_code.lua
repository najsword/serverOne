...
AUTH_ERROR = {
	...
	subid_nil 		   = add{code = 0x010e, desc = "subid为空"},
	auth_info_expire   = add{code = 0x0110, desc = "auth_info过期"},
}

ROLE_ERROR = {
	role_nil        = add{code = 0x0201, desc = "角色为空"},
	password_nil       = add{code = 0x0202, desc = "密码为空"},
	telephone_nil      = add{code = 0x0203, desc = "电话号码为空"},
	role_exist      = add{code = 0x0204, desc = "帐号存在"},
	role_id_limit    = add{code = 0x0205, desc = "玩家id已达上限"},
	repeat_login       = add{code = 0x0206, desc = "重复登录"},
	account_not_exist  = add{code = 0x0207, desc = "不存在此帐号"},
	password_wrong     = add{code = 0x0208, desc = "密码错误"},
	role_not_exist   = add{code = 0x0209, desc = "对应的玩家不存在"},
	union_id_nil       = add{code = 0x020a, desc = "union_id为空"},
	head_url_nil       = add{code = 0x020b, desc = "头像地址为空"},
	nickname_nil       = add{code = 0x020c, desc = "昵称为空"},
	sex_nil            = add{code = 0x020d, desc = "性别为空"},
}

AREA_ERROR = {
	room_not_exist        = add{code = 0x0301, desc = "房间为空"},
	room_forbid_enter     = add{code = 0x0302, desc = "房间禁止进入"},
	gold_not_enough     = add{code = 0x0303, desc = "金币不足"},
	room_not_enough     = add{code = 0x0304, desc = "房间不足够"},
}

CHAT_ERROR = {
	chat_param_error        = add{code = 0x0401, desc = "参数为空"},
}
...
GAME_ERROR = {
	...
	enter_room_double = add{code = 0x0d10, desc = "重复进入房间两次"},
	no_in_hall = add{code = 0x0d11, desc = "玩家不在大厅"},
	enter_room_double_session = add{code = 0x0d12, desc = "不同session重复进入房间两次"},
}
